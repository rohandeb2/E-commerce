...
type AddToCartWrapperProps = {
  // Only the cart-relevant fields are required here.
  // Other product fields (rating, amount, originalId, colors, etc.) may be present
  // but they are not needed to add an item to the cart.
  product: Pick<
    CartItem,
    "_id" | "title" | "price" | "image" | "unit_of_measure" | "shop_category"
  > & {
    // Optional fields accepted by callers (not required for cart)
    description?: string;
    categories?: string[];
    rating?: number;
    amount?: number;
