  ...
  const placeOrder = async () => {
    try {
      ...
      const orderItems = cartItems.map((item) => {
        const productId = item._id || (item as any).product || (item as any).productId || (item as any).id;
        if (!productId) {
          throw new Error(`Order validation failed: missing product id for cart item ${item.title || ''}`);
        }
